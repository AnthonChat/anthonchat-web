...
-   **Columns**:
    ...
    -   `channel_id` (text): Foreign Key -> public.channels.id
    -   `link` (text): The unique identifier on the platform (e.g., phone number or chat ID).
    -   `created_at` (timestamp with time zone)
...
-   **Columns**:
    ...
    -   `user_id` (uuid): Foreign Key -> public.users.id
    -   `channel_id` (text): Foreign Key -> public.channels.id
    -   `nonce` (uuid): Unique verification code.
