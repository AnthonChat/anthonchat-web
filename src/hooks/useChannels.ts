...
interface UseChannelsActions {
  ...
  createChannel: (
    data: Partial<UserChannelWithChannel>
  ) => Promise<UserChannelWithChannel | null>;
  updateChannel: (
    id: string,
    data: Partial<UserChannelWithChannel>
  ) => Promise<UserChannelWithChannel | null>;
...
export function useChannels(
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      ...
      const transformedData: UserChannelWithChannel[] =
        data?.map((item) => ({
          id: item.id,
          link: item.link,
    ...
    const setupRealtimeSubscription = () => {
      channel = supabase
        ...
        .on(
          ...
          (payload) => {
            hookLogger.info("REALTIME_CHANNEL_UPDATE", "CHANNELS", {
              payload,
              userId,
            });
            ...
            switch (payload.eventType) {
              case "INSERT":
                setChannels((prev) => [
                  ...prev,
                  payload.new as UserChannelWithChannel,
                ]);
                break;
              case "UPDATE":
                setChannels((prev) =>
