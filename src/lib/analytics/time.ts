...
export function resolvePresetOrRange(presetOrRange: TimePreset | DateRange, now = new Date()): DateRange {
  if (typeof presetOrRange === 'string') return resolveRange(presetOrRange, now)
...
export function coerceRangeFromSearchParams(sp: SearchParams, key: string, defaultPreset: TimePreset): { range: DateRange; preset: string } {
  const get = (k: string) => {
    const v = sp[k]
...
export function rangeFilter<T extends { created_at?: string }>(
  q: any,
  column: string,
  range: DateRange
) {
