...
export async function getSignupsAttribution(
  ...
  const explicitWebsite = users.filter(
    (u) => u.signup_source === "website"
  ).length;
...
export async function getMessagesPerChannel(
  preset: TimePreset | DateRange = "7d"
  ...
  const queries = channels.map((ch) => {
    let q = supabase
      .from("chat_messages")
      .select("id, user_channels!inner(channel_id)", {
        count: "exact",
        head: true,
      })
  ...
  const result = channels.map((channel_id) => ({
    channel_id,
    count: counts[channel_id] ?? 0,
  }));
...
export async function getUsersPerChannel(
  preset: TimePreset | DateRange | "lifetime" = "lifetime"
): Promise<UsersPerChannel[]> {
  const supabase = createServiceRoleClient();
  ...
  return Object.entries(userSets).map(([channel_id, set]) => ({
 ...
 */
export async function getNewVsReturningActive(
  preset: TimePreset | DateRange = "7d"
): Promise<NewReturningActive> {
  const supabase = createServiceRoleClient();
  const range = resolvePresetOrRange(preset);
  const endIso = range.end ?? new Date().toISOString();
  ...
  for (let i = 0; i < ids.length; i += chunkSize) {
    ...
    while (true) {
      ...
      for (const row of rows) {
        const rel = (row as { user_channels?: unknown })
          .user_channels as unknown;
        const uid = extractUserIdFromRelation(rel);
  ...
  const returningActive = Array.from(activeSet).reduce(
    (acc, uid) => acc + (beforeSet.has(uid) ? 1 : 0),
    0
  );
...
export async function getActivationMetrics(
  ...
  for (const u of signupRows ?? []) {
    ...
    const ts = new Date(
      String((u as { created_at: unknown }).created_at)
    ).getTime();
...
type ChatMsgWithUser = {
  created_at?: unknown;
  user_channels?: UserChannelsRel;
};
...
function extractUserIdFromRelation(rel: unknown): string | null {
  ...
  if (Array.isArray(rel)) {
    ...
    return typeof first?.user_id === "string" && first.user_id
      ? first.user_id
      : null;
  }
 ...
 * value = DAU / MAU
 */
export async function getStickinessOverTime(
  preset: TimePreset | DateRange = "7d",
  bucketHours = 24
  ...
  while (true) {
    ...
    if (error)
      throw new Error(
        `getStickinessOverTime error: ${serializeErrorForMessage(error)}`
      );
...
async function getNewReturningOverTime(
  preset: TimePreset | DateRange = "7d",
  bucketHours = 24
): Promise<NVRBucketPoint[]> {
...
export async function getMessagesPerUserDistribution(
  ...
  while (true) {
    ...
    if (error)
      throw new Error(
        `getMessagesPerUserDistribution error: ${serializeErrorForMessage(
          error
        )}`
      );
  ...
  for (let i = 1; i < binCount; i++) {
    ...
    const idx = Math.min(
      totalUsers - 1,
      Math.max(0, Math.floor(p * totalUsers) - 1)
    );
  ...
  for (const cnt of counts) {
    const idx = bins.findIndex(
      (b) => cnt >= b.from && (b.to === null ? true : cnt <= b.to)
    );
...
export async function getPowerUserConcentration(
  ...
  const percentiles = (opts?.percentiles ?? [0.01, 0.05, 0.1])
    .map((p) => Math.max(0.001, Math.min(0.5, p)))
    .sort((a, b) => a - b);
  ...
  while (true) {
    ...
    if (error)
      throw new Error(
        `getPowerUserConcentration error: ${serializeErrorForMessage(error)}`
      );
  ...
  if (n === 0 || totalMessages === 0)
    return percentiles.map((p) => ({ percentile: p, share: 0 }));
...
export async function getEngagementVelocity(
  ...
  while (true) {
    ...
    if (error)
      throw new Error(
        `getEngagementVelocity error: ${serializeErrorForMessage(error)}`
      );
 ...
 */
export async function getWeekdaySeasonality(
  preset: TimePreset | DateRange = "7d"
  ...
  while (true) {
    ...
    if (error)
      throw new Error(
        `getWeekdaySeasonality error: ${serializeErrorForMessage(error)}`
      );
...
> {
  ...
  const buckets = (function makeBuckets(r: DateRange, hours: number) {
    const { start, end } = (function ensure(
      range: DateRange,
      fallbackDays = 30
    ) {
      const endIso = range.end ?? new Date().toISOString();
  ...
  const windowStartIso =
    buckets[0].start ?? range.start ?? new Date().toISOString();
  const windowEndIso =
    buckets[buckets.length - 1].end ?? range.end ?? new Date().toISOString();
  const windowStartTs = new Date(windowStartIso).getTime();
  const windowEndTs = new Date(windowEndIso).getTime();

  type Row = {
    created_at?: unknown;
    user_channels?: { user_id?: string } | Array<{ user_id?: string }>;
  };
  const points: { t: number; uid: string }[] = [];
  ...
  // 1) Fetch all messages in [windowStart, windowEnd) once
  {
    ...
    while (true) {
      ...
      if (error)
        throw new Error(
          `getNewReturningOverTimeOptimized (window) error: ${serializeErrorForMessage(
            error
          )}`
        );
  ...
  const activePerBucket: Array<Set<string>> = buckets.map(
    () => new Set<string>()
  );
  ...
  for (const p of points) {
    ...
    const idx = Math.min(
      ...
      Math.max(
        0,
        Math.floor(
          (p.t - windowStartTs) / Math.max(1, bucketHours * 60 * 60 * 1000)
        )
      )
    );
  ...
  if (unionActive.size > 0) {
    ...
    for (let i = 0; i < ids.length; i += chunkSize) {
      ...
      while (true) {
        ...
        if (error)
          throw new Error(
            `getNewReturningOverTimeOptimized (prior) error: ${serializeErrorForMessage(
              error
            )}`
          );
        const rows = (data ?? []) as Row[];
  ...
  for (let i = 0; i < buckets.length; i++) {
    ...
    const date =
      buckets[i].start ??
      buckets[i].end ??
      new Date(windowStartTs + i * bucketHours * 60 * 60 * 1000).toISOString();
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function getGrowthDecomposition(
  preset: TimePreset | DateRange = "7d",
  bucketHours = 24,
  ...
  while (true) {
    ...
    if (error)
      throw new Error(
        `getGrowthDecomposition error: ${serializeErrorForMessage(error)}`
      );
...
export async function getCohortRetentionHeatmap(
  ...
  if (usersErr)
    throw new Error(
      `getCohortRetentionHeatmap (users) error: ${serializeErrorForMessage(
        usersErr
      )}`
    );
  ...
  for (let i = 0; i < userIds.length; i += chunkSize) {
    ...
    while (true) {
      ...
      if (error)
        throw new Error(
          `getCohortRetentionHeatmap (messages) error: ${serializeErrorForMessage(
            error
          )}`
        );
      const rows = (data ?? []) as ChatMsgWithUser[];
...
export async function getRetentionCurvesByCohort(
  ...
  if (usersErr)
    throw new Error(
      `getRetentionCurvesByCohort (users) error: ${serializeErrorForMessage(
        usersErr
      )}`
    );
  ...
  for (let i = 0; i < userIds.length; i += chunkSize) {
    ...
    while (true) {
      ...
      if (error)
        throw new Error(
          `getRetentionCurvesByCohort (messages) error: ${serializeErrorForMessage(
            error
          )}`
        );
      const rows = (data ?? []) as ChatMsgWithUser[];
...
export async function getFirst24hFunnel(
  ...
  if (usersErr)
    throw new Error(
      `getFirst24hFunnel (users) error: ${serializeErrorForMessage(usersErr)}`
    );
  ...
  for (let i = 0; i < userIds.length; i += chunkSize) {
    ...
    while (true) {
      ...
      if (error)
        throw new Error(
          `getFirst24hFunnel (messages) error: ${serializeErrorForMessage(
            error
          )}`
        );
      const rows = (data ?? []) as ChatMsgWithUser[];
  ...
  for (const uid of userIds) {
    const s = signupAt.get(uid)!;
    const ts = (byUserTs.get(uid) ?? []).filter(
      (t) => t >= s && t < s + 24 * 60 * 60 * 1000
    );
...
export async function getSessionizationMetrics(
  ...
  while (true) {
    ...
    if (error)
      throw new Error(
        `getSessionizationMetrics error: ${serializeErrorForMessage(error)}`
      );
  ...
  for (const b of buckets) {
    ...
    const avgMin =
      totalSessions > 0 ? totalDurationMs / totalSessions / (60 * 1000) : 0;

    sessionsPerUser.push({ date: new Date(ts).toISOString(), value: spu });
    avgSessionMinutes.push({
...
export async function getReactivationRate(
  ...
  while (true) {
    ...
    if (error)
      throw new Error(
        `getReactivationRate error: ${serializeErrorForMessage(error)}`
      );
