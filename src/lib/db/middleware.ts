...
export async function updateSession(request: NextRequest) {
  ...
  middlewareLogger.info(
    ...
    "SUPABASE_MIDDLEWARE_PROCESSING",
    { pathname },
  ...
  if (!claims && !publicPaths.some((path) => pathname.startsWith(path))) {
    middlewareLogger.info(
      ...
      "SUPABASE_MIDDLEWARE_REDIRECT_TO_LOGIN",
      { pathname }
    );
  ...
  if (claims) {
    ...
    middlewareLogger.info(
      ...
      "SUPABASE_MIDDLEWARE_USER_DATA_FETCH",
      { userData, userError },
      claims.claims.sub
    );
    ...
    if (userError || !userData) {
      middlewareLogger.error(
        ...
        "SUPABASE_USER_ONBOARDING_STATUS_FETCH_ERROR",
        { error: userError },
        claims.claims.sub
      );
    ...
    middlewareLogger.info(
      ...
      "SUPABASE_MIDDLEWARE_ONBOARDING_STATUS",
      { onboardingComplete },
      claims.claims.sub
    );
    ...
    if (!onboardingComplete) {
      ...
      if (
        ...
        middlewareLogger.info(
          ...
          "SUPABASE_MIDDLEWARE_REDIRECT_TO_SIGNUP_COMPLETE",
          { pathname },
          claims.claims.sub
        );
        const url = request.nextUrl.clone();
      ...
      if (pathname.startsWith("/signup")) {
        middlewareLogger.info(
          ...
          "SUPABASE_MIDDLEWARE_REDIRECT_TO_DASHBOARD",
          { pathname },
          claims.claims.sub
        );
        const url = request.nextUrl.clone();
