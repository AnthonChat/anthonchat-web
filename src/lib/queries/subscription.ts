...
type StripeSubscription =
  StripeDatabase["stripe"]["Tables"]["subscriptions"]["Row"];
 ...
 */
function extractSubscriptionItemDetails(
  ...
  try {
    ...
    if (subscription.plan && typeof subscription.plan === "string") {
      const planData = JSON.parse(
        subscription.plan
      ) as StripeSubscriptionItemPrice;
      return [
        {
          id: planData.id,
          price: planData,
 ...
 */
async function buildSubscriptionResult(
  subscription: StripeSubscription
  ...
  if ((!currentPeriodStart || !currentPeriodEnd) && items.length > 0) {
    try {
      ...
      if (typeof itemsJson === "string") {
        ...
        if (
          parsedItems.data &&
          Array.isArray(parsedItems.data) &&
          parsedItems.data.length > 0
        ) {
        ...
        if (Array.isArray(itemsData) && itemsData.length > 0) {
          const firstItem = itemsData[0] as {
            current_period_start?: number;
            current_period_end?: number;
          };
          if (firstItem.current_period_start && !currentPeriodStart) {
