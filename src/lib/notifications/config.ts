...
export const ERROR_MESSAGES = {
  [NotificationErrorType.VERIFICATION_POLLING_ERROR]: {
    ...
    userSuggestion:
      "Assicurati di aver completato l'azione richiesta e riprova.",
  },
  [NotificationErrorType.VERIFICATION_TIMEOUT]: {
...
/**
 * Configurazione per il polling di verifica (internal - currently unused)
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const VERIFICATION_POLLING_CONFIG = {
...
export function getErrorTypeConfig(
  ...
  return errorType in NOTIFICATION_CONFIG.errorDefaults
    ? NOTIFICATION_CONFIG.errorDefaults[
        errorType as keyof typeof NOTIFICATION_CONFIG.errorDefaults
      ]
    : {};
}
...
/**
 * Utility per determinare se siamo in modalità debug (internal - currently unused)
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function isDebugMode(): boolean {
  const env = process.env.NODE_ENV;
  return env === "development"
    ? NOTIFICATION_CONFIG.development.debugMode
    : NOTIFICATION_CONFIG.production.debugMode;
}

/**
 * Utility per determinare se mostrare errori dettagliati (internal - currently unused)
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function shouldShowDetailedErrors(): boolean {
  const env = process.env.NODE_ENV;
  return env === "development"
    ? NOTIFICATION_CONFIG.development.showDetailedErrors
    : NOTIFICATION_CONFIG.production.showDetailedErrors;
}
