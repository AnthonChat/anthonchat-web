...
import { SubscriptionCardSkeleton, LoadingWrapper } from "@/components/ui/loading";
import {
	Sheet,
	SheetContent,
	SheetDescription,
			...
			<SheetContent className="w-[400px] sm:w-[540px] flex flex-col h-full">
				<SheetHeader className="space-y-4 px-6 pt-4 pb-3 flex-shrink-0">
					<div className="flex items-start gap-3">
						...
						<div className="flex-1 min-w-0">
							<SheetTitle className="text-xl font-bold leading-tight mb-1">{title} Usage Details</SheetTitle>
							<SheetDescription className="text-sm text-muted-foreground">
								Comprehensive breakdown of your current {title.toLowerCase()} usage and limits.
							</SheetDescription>
						</div>
					</div>
				...
				<div className="px-6 pb-4 space-y-5 flex-1 overflow-y-auto">
						...
						<CardContent className="space-y-4 px-6 pb-4">
							...
							<div className="space-y-2">
								<div className="flex justify-between items-center">
									...
									<span className="text-muted-foreground font-medium text-sm">{safePercent.toFixed(1)}%</span>
								</div>
								<div className="h-4 bg-border rounded-full overflow-hidden">
									<div 
										className={cn("h-full transition-all duration-500 rounded-full relative", getProgressColor(safePercent))}
							...
							<div className="grid grid-cols-2 gap-6">
								...
								<div className="space-y-1 text-center">
									...
									<div className={cn("text-lg font-bold", remaining < 0 ? "text-destructive" : "text-success")}>
					...
					<Card className="shadow-sm">
						...
						<CardContent className="space-y-2 px-6 pb-4">
							<div className="flex items-center justify-between py-2 border-b border-border/50">
								...
								<span className="font-bold text-base">{safeLimit.toLocaleString()}</span>
							</div>
							<div className="flex items-center justify-between py-2 border-b border-border/50">
								<span className="text-sm font-medium">Currently Used</span>
								<span className="font-bold text-base">{safeUsed.toLocaleString()}</span>
							</div>
							<div className="flex items-center justify-between py-2 border-b border-border/50">
								<span className="text-sm font-medium">Remaining</span>
								<span className={cn("font-bold text-base", remaining < 0 ? "text-destructive" : "text-success")}>
					...
					{(isNearLimit || isOverLimit) && (
						<Card className={cn("border-2 shadow-sm", isOverLimit ? "border-destructive/50 bg-destructive/5" : "border-warning/50 bg-warning/5")}>
				<CardHeader className="pb-3">
					<CardTitle className={cn("text-base font-semibold flex items-center gap-2", isOverLimit ? "text-destructive" : "text-warning")}>
								...
								{isOverLimit ? (
									<>
										<p className="text-sm text-destructive font-medium">
											You have exceeded your {title.toLowerCase()} limit. Your service may be restricted.
										</p>
										<p className="text-xs text-muted-foreground">
									...
									<>
										<p className="text-sm text-warning font-medium">
											You&apos;re approaching your {title.toLowerCase()} limit.
										</p>
										<p className="text-xs text-muted-foreground">
											Monitor your usage closely or consider upgrading to avoid hitting the limit.
										</p>
									</>
								)}
							</div>
