 ...
 */
export const AccessibilityTrigger = forwardRef<HTMLButtonElement, AccessibilityTriggerProps>(
  ({ onClick, isOpen, className, position = 'bottom-right' }, ref) => {
    ...
    return (
      <Button
        ...
        aria-label={
          mounted
            ? `Accessibility settings. Current: ${locale.toUpperCase()}, ${currentTheme ?? 'system'} theme. ${isOpen ? 'Menu open' : 'Click to open menu'}`
            : `Accessibility settings. Current: ${locale.toUpperCase()}. ${isOpen ? 'Menu open' : 'Click to open menu'}`
        }
