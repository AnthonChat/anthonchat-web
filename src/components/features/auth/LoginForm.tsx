...
export default function LoginForm({
  ...
  const validateField = (field: keyof LoginFields, value: string) => {
    const parsed = LoginSchema.pick({ [field]: true } as Record<
      keyof LoginFields,
      true
    >).safeParse({ [field]: value } as Partial<LoginFields>);
    if (!parsed.success) {
  ...
  const getChannelLinkingFeedback = (): ChannelLinkingFeedback | null => {
    ...
    if (
      ...
      searchParams.get("channel_linked") === "true"
    ) {
      return {
        type: "success",
        title: "Canale collegato con successo",
        message: `Il tuo canale ${
          channel || "chat"
        } è stato collegato durante la registrazione. Accedi per continuare.`,
    ...
    if (
      searchParams.get("from") === "signup" &&
      searchParams.get("channel_error") === "true"
    ) {
      return {
        type: "error",
        title: "Errore nel collegamento del canale",
        message: `Non è stato possibile collegare automaticamente il tuo canale ${
          channel || "chat"
        }. Potrai collegarlo manualmente dopo l'accesso.`,
    ...
    if (
      ...
      searchParams.get("link_expired") === "true"
    ) {
      return {
        type: "expired",
        title: "Link scaduto",
        message:
          "Il link di registrazione è scaduto. Accedi al tuo account e potrai richiedere un nuovo link per collegare il canale.",
    ...
    if (
      ...
      return {
        ...
        message: `Accedi per collegare il tuo canale ${
          channel ? channel.charAt(0).toUpperCase() + channel.slice(1) : "chat"
        } al tuo account.`,
  ...
  useEffect(() => {
    ...
    if (message === "account_exists" && (channel || link)) {
      ...
      if (prev?.type !== "account_exists" || prev.value !== accountExistsKey) {
        showSuccess(
          ...
          `Hai già un account con questa email. Accedi per collegare il tuo canale ${
            channel
        ...
        lastShownNotificationRef.current = {
          type: "account_exists",
          value: accountExistsKey,
        };
      }
    ...
    if (message && message !== "account_exists") {
      if (prev?.type !== "external_error" || prev.value !== message) {
        showError("Errore di autenticazione", message, {
    ...
    if (formState.message && !formState.success) {
      if (prev?.type !== "form_error" || prev.value !== formState.message) {
        showError("Errore durante il login", formState.message, {
          errorType: NotificationErrorType.AUTH_UNAUTHORIZED,
      ...
      <Card className="shadow-lg">
        <CardHeader className="text-center space-y-3 pb-6">
          ...
          <CardDescription className="text-sm sm:text-base">
            {channel || link ? (
              <>
                Accedi per collegare il tuo canale{" "}
                {channel &&
                  `${channel.charAt(0).toUpperCase() + channel.slice(1)}`}
              </>
            ) : (
          ...
          {channelFeedback && (
            <Alert
              className={`mt-4 ${
                channelFeedback.type === "success"
                  ? "border-green-200 bg-green-50"
                  : channelFeedback.type === "error"
                  ? "border-red-200 bg-red-50"
                  : channelFeedback.type === "expired"
        ...
        <form action={formAction} onSubmit={onSubmit} noValidate>
          <CardContent className="flex flex-col w-full gap-6 text-foreground px-6">
            ...
            {(channel || link) && (
              <input type="hidden" name="autoLinkChannel" value="true" />
            )}
            {redirectTo && (
            ...
            {isPending && (
              <div className="space-y-4 p-4 bg-primary/5 rounded-lg border border-primary/20">
                ...
                <p className="text-xs text-muted-foreground">
                  Stiamo verificando le tue credenziali e preparando la
                  sessione.
                </p>
              </div>
            )}
