          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 text-left">
              ...
              <div className="text-sm font-medium text-muted-foreground mt-1">
                {formatChannelLink(channelDetails.id, userChannel.link)}
              </div>
            </div>
          </div>
            ...
            {isVerified ? (
              <>
                ...
                <Badge variant="success">
                  {t("channelsOverview.badges.verified")}
                </Badge>
              </>
            ) : (
              <>
                ...
                <Badge variant="warning">
                  {t("channelsOverview.badges.unverified")}
                </Badge>
              </>
            )}
          </div>
...
function ChannelsOverviewContent({ userId }: ChannelsOverviewProps) {
  ...
  const getChannelIcon = React.useCallback(
    (channelId: string): React.ReactElement => {
  ...
  const formatChannelLink = React.useCallback(
    (channelId: string, link: string) => {
      switch (channelId.toLowerCase()) {
        case "telegram":
          return `@${link}`;
        default:
          ...
          skeleton={
            <div className="space-y-4">
              {[...Array(2)].map((_, i) => (
                <div
                  key={i}
                  className="p-5 rounded-xl bg-card border-2 border-border"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
            ...
            {channels && channels.length > 0 ? (
              ...
              <div className="text-center py-12 animate-fade-in">
                ...
                <h3 className="text-xl font-bold text-foreground mb-3">
                  {t("channelsOverview.emptyTitle")}
                </h3>
                <p className="text-base font-medium text-muted-foreground mb-8">
                  {t("channelsOverview.emptyDescription")}
                </p>
              </div>
            )}
          </div>
...
export const ChannelsOverview = React.memo(function ChannelsOverview(
  props: ChannelsOverviewProps
) {
  return (
    <ErrorBoundary
      fallback={
