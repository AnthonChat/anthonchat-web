...
const saveVerificationStates = (states: Record<string, ChannelVerificationState>) => {
  try {
  ...
  try {
    ...
    if (stored) {
      ...
      Object.entries(states).forEach(([channelId, state]) => {
        ...
        else if (typedState.status === 'pending' && typedState.startedAt && (now - typedState.startedAt) < 10 * 60 * 1000) {
          cleanStates[channelId] = typedState
        }
...
export function AddChannelForm({ availableChannels, existingChannels }: AddChannelFormProps) {
  ...
  const initializeChannelStates = useCallback(() => {
    ...
    availableChannels.forEach((channel) => {
      ...
      } else if (persistedState && persistedState.status === 'pending' && persistedState.nonce) {
  ...
  const [channelStates, setChannelStates] = useState<Record<string, ChannelVerificationState>>(initializeChannelStates)
  const [pollingIntervals, setPollingIntervals] = useState<Record<string, NodeJS.Timeout>>({})
  const [timeoutIntervals, setTimeoutIntervals] = useState<Record<string, NodeJS.Timeout>>({})
  const pollingInitialized = useRef(false)
  const router = useRouter()

  const updateChannelState = useCallback((channelId: string, updates: Partial<ChannelVerificationState>) => {
  ...
  const startPolling = useCallback((channelId: string, nonce: string) => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (response.ok && data.status === 'done') {
          ...
          updateChannelState(channelId, {
            status: 'expired',
            error: data.error || 'Verification link has expired. Please try again.'
          })
          clearInterval(interval)
          clearTimeout(timeout)
          setPollingIntervals(prev => {
  ...
  const getChannelIcon = (channelId: string) => {
	...
	default:
		return <MessageCircle className="h-12 w-12 mx-auto mb-4 text-muted-foreground" />
    }
  }
  ...
  const startVerification = async (channel: Channel) => {
    ...
    updateChannelState(channel.id, { status: 'pending', error: undefined, startedAt })
  ...
  const getErrorMessage = (state: ChannelVerificationState, channelName: string) => {
    if (state.status === 'expired') {
      return {
        title: `${channelName} Verification Expired`,
        message: 'The verification link expired after 5 minutes. This is normal for security reasons.',
      ...
      return {
        ...
        message: state.error || 'An unexpected error occurred during verification.',
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h2 className="text-2xl font-semibold text-foreground">Choose Channel Type</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {availableChannels.map((channel) => {
              ...
              const hasError = state.status === 'error' || state.status === 'expired'
              ...
              if (isConnected) {
                ...
                cardClassName += 'border-warning/50 bg-warning/5 hover:bg-warning/10'
              } else if (state.status === 'error') {
                cardClassName += 'border-destructive/50 bg-destructive/5 hover:bg-destructive/10'
              } else {
                ...
                <Card key={channel.id} className={cardClassName}>
                  <CardContent className="p-6 text-center">
                    ...
                    <div className="flex items-center justify-center gap-2 mb-2">
                      <h3 className="font-semibold">{getChannelName(channel.id)}</h3>
                    ...
                    {isConnected && (
                      <div className="mb-4 p-3 bg-success/10 rounded-lg">
                        <p className="text-sm text-success">
                          Connected as: <span className="font-mono">{state.link}</span>
                        </p>
                      </div>
                    )}
                    ...
                    {hasError && (
                      <div className="mb-4">
                        {(() => {
                          const errorInfo = getErrorMessage(state, getChannelName(channel.id))
                          if (!errorInfo) return null

                          const isExpired = state.status === 'expired'
                          const bgColor = isExpired ? 'bg-warning/5' : 'bg-destructive/5'
                          const borderColor = isExpired ? 'border-warning/20' : 'border-destructive/20'
                          const textColor = isExpired ? 'text-warning' : 'text-destructive'
                          const iconColor = isExpired ? 'text-warning' : 'text-destructive'

                          return (
                            <div className={cn("p-4 rounded-lg border space-y-3", bgColor, borderColor)}>
                              ...
                              {isExpired && state.startedAt && (
                                <div className={cn("text-xs opacity-70 pt-2 border-t border-warning/20", textColor)}>
                    ...
                    <div className="space-y-2">
                      ...
                      {hasError && (
                        <Button 
                          className="w-full" 
                          onClick={() => retryVerification(channel)}
                          variant={state.status === 'expired' ? 'default' : 'destructive'}
                        >
