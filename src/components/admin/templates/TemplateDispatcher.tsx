...
export function TemplateDispatcher({
	...
	const handleSend = async () => {
		...
		const trimmedParameterValues = selectedTemplate.parameters.map(
			(param) => {
			...
			<div className="grid gap-4 md:grid-cols-[2fr,1fr]">
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Filter users before selecting recipients for Meta
							templates.
						</CardDescription>
					</CardHeader>
						...
						<div className="space-y-2">
							...
							<div className="flex flex-wrap gap-2">
								{channelOptions.map((channel) => {
									const selected = selectedChannels.includes(
										channel.id
									);
									return (
										<Badge
						...
						<div className="flex items-center justify-between border-t pt-4">
							...
							<div className="flex items-center gap-2">
								...
								<span className="text-xs text-muted-foreground">
									...
									{Math.max(
										1,
										Math.ceil(totalRecipients / limit)
									)}
								</span>
								<Button
									variant="outline"
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Pick a Meta template and fill placeholders.
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-3">
						{templates.length === 0 && (
							<p className="text-sm text-muted-foreground">
								No templates available. Check Meta credentials
								or try again later.
							</p>
						)}
								...
								{selectedTemplate?.parameters.length ? (
									<div className="space-y-4">
										{selectedTemplate.parameters.map(
											(parameter) => {
												const isMediaParam = Boolean(
													parameter.mediaFormat
												);
												const defaultValue =
													parameter.defaultMediaHandle;
												const isScontent =
										...
										<TableRow key={recipient.userId}>
											...
											<TableCell>
												<div className="text-xs text-muted-foreground">
													{
														recipient.totalRequestsUsed
													}{" "}
													req /{" "}
													{recipient.totalTokensUsed}{" "}
													tokens
													{whatsappChannel && (
								...
								{recipients.length === 0 && !loading && (
									<TableRow>
										<TableCell
											...
											No recipients match the current
											filters.
										</TableCell>
									</TableRow>
								)}
