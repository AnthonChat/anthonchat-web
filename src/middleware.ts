 ...
 */
function matchesRoute(pathnameWithoutLocale: string, routes: string[]): boolean {
...
export default async function middleware(req: NextRequest) {
  ...
  const effectiveLocale = pathLocale && isSupportedLocale(pathLocale) ? pathLocale : defaultLocale;
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      if (error) supabaseResponse.headers.set('x-auth-error', String(error.message).slice(0, 120));
    }
